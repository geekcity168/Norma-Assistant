...
// Retrieve the latest bank details for the user
$sql = "SELECT bank_name, account_number FROM banks WHERE user_id = ? ORDER BY bank_id DESC LIMIT 1";
$stmt = $conn->prepare($sql);
if (!$stmt) {
    error_log("Prepare failed: " . $conn->error);
    die("Prepare failed: " . $conn->error);
...
if ($result->num_rows === 0) {
    echo json_encode(['error' => 'No bank details found']);
    exit;
}

$bankDetails = $result->fetch_assoc();
$bankDetails['full_name'] = $fullName;
